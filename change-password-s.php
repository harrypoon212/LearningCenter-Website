...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	*/
	...
	$qs = "SELECT * FROM staff WHERE staff_ID = '$_SESSION[staff_ID]' and staff_password = '$originalPassword'";
	$query = mysqli_query($conn, $qs) or die(mysqli_error($conn));

	if (mysqli_num_rows($query) == 1){
		if ($newPassword1==$newPassword2){
			$qs = "UPDATE staff SET staff_password = '$newPassword1' WHERE staff_ID = '$_SESSION[staff_ID]';";
			$error_message.=$qs."<br>";
			$query = mysqli_query($conn, $qs) or die(mysqli_error($conn));
