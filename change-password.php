...
//joey
 if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	*/
	...
	$qs = "SELECT * FROM student WHERE student_ID = '$_SESSION[student_ID]' and student_password = '$originalPassword'";
	$query = mysqli_query($conn, $qs) or die(mysqli_error($conn));

	if (mysqli_num_rows($query) == 1){
		if ($newPassword1==$newPassword2){
			$qs = "UPDATE student SET student_password = '$newPassword1' WHERE student_ID = '$_SESSION[student_ID]';";
			$error_message.=$qs."<br>";
			$query = mysqli_query($conn, $qs) or die(mysqli_error($conn));
